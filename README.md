...
- A second layer under the path scope, because a denied topic still gets mentioned in passing on allowed pages: `EXOCORTEX_PUBLIC_REDACT` is a comma-separated list of blocked terms (list the variants yourself: `take3, take 3, take three`). On the public token, a page whose path, title, or description mentions a term is not-found and absent from search and listings; on every other page the paragraph, list item, table row, quote line, fenced block, or titled section (heading through the next heading at its level) that mentions a term is dropped before the page is served, and search neither matches on the term nor shows it in a snippet. Nothing marks a removal. Matching is case-insensitive on whole words, so `trep` never fires inside `entrepreneur`. The model only says what it reads, which is why the filter sits here and not on the model's output.
...
Two things this tier deliberately does not do, and where they live instead: it does not understand topics beyond the term list (a page can talk around a blocked name without using it; the calling site's system prompt and its own tests own that seam), and it does not rate-limit (the site's edge does).
...
>
> Retrieval. When a question touches my life, work, projects, people, or history, or when context about me would change your answer, search the exocortex with query_wiki before answering from your built-in memory or general knowledge. Use get_page to read full pages from hits, and mention which page you are drawing from (for example, "per wiki/projects/my-project.md"). Results carry a status field. Verified pages are human-confirmed. Draft pages are machine-written and may contain inference, so calibrate your confidence accordingly. If the exocortex and your built-in memory disagree, the exocortex wins. If the exocortex and I disagree in chat, I win, and that is worth capturing as a correction.
>
> Capture. When I say "remember this," state a decision, correct something you got wrong about me, or share something durably worth keeping (a preference, a plan, a fact about my life), save it with capture_to_inbox. Give it a clear title, the substance in markdown, and provenance like "the user, in conversation, [date]". Corrections to existing knowledge are new capture entries, never described as edits. Do not capture small talk or transient logistics. Capture what would matter in a month. Tell me in one line when you have captured something.
>
> Limits. Captures reach the vault only when the consumer job runs on the machine that owns it. Check your own schedule and state it here, since the honest cadence is what stops an agent hedging wrongly: the reference deployment consumes inside an hourly job that only fires while the laptop is awake, so a capture from minutes ago may not be searchable yet and recent events may be missing entirely. Say so rather than concluding something does not exist. If the connector is unavailable in a chat, say so and fall back to built-in memory rather than guessing.
